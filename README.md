...
> 🎯 **面试演示模式**：本项目新增了演示功能模块，可通过前端配置绕过后端权限控制，方便面试展示。详见 [演示模式说明](#演示模式---绕过后端权限控制详解)
...
> 以下文件为面试演示功能新增，使用 Mock 数据，无需后端支持
...
所有演示页面使用的模拟数据统一存放在 `src/mock/index.ts` 中：
...
> ⚠️ **注意事项**
> 1. **演示模式开关**：如需恢复正常权限控制，将 `src/global/demo-mode.ts` 中的 `DEMO_MODE` 设为 `false`
